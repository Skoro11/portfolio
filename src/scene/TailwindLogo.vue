...
const meshes = shallowRef<{ geo: THREE.ExtrudeGeometry; mat: THREE.Material; outline: boolean }[]>([]);
...
const mainMat = new THREE.MeshStandardMaterial({ map: diffuse, color: "#ffffff", roughness: 0.9, metalness: 0 });
...
onMounted(() => {
  ...
  loader.load("/tailwind-css-2.svg", (data) => {
    ...
    meshes.value = [
      ...geos.map(geo => ({ geo, mat: mainMat as THREE.Material, outline: false })),
      ...geos.map(geo => ({ geo, mat: outlineMat as THREE.Material, outline: true })),
    ];
  });
});
</script>
