...
const meshes = shallowRef<{ geo: THREE.ExtrudeGeometry; mat: THREE.Material; outline: boolean; posZ: number; rotZ?: number }[]>([]);
...
const mainMat    = new THREE.MeshStandardMaterial({ map: diffuse, normalMap, roughnessMap: roughMap, color: "#aaaaaa", metalness: 0 });
const backMat    = new THREE.MeshStandardMaterial({ map: backDiffuse, color: "#ffffff", roughness: 0.9, metalness: 0 });
const outlineMat = new THREE.MeshStandardMaterial({ map: outerDiffuse, normalMap: outerNormal, roughnessMap: outerRough, color: "#aaaaaa", side: THREE.BackSide, metalness: 0 });
  ...
  loader.load("/git-icon.svg", (data) => {
    ...
    data.paths.forEach((path) => {
      SVGLoader.createShapes(path).forEach((shape) => {
        geos.push(new THREE.ExtrudeGeometry(shape, { depth: 8, bevelEnabled: false }));
    ...
    roundedRect.lineTo(-w, -h + r);
    roundedRect.quadraticCurveTo(-w, -h, -w + r, -h);
    const squareGeo = new THREE.ExtrudeGeometry(roundedRect, { depth: 8, bevelEnabled: false });
    const backZ = s.z / 2 + 1;

    meshes.value = [
      { geo: squareGeo, mat: backMat as THREE.Material, outline: false, posZ: backZ, rotZ: Math.PI / 4 },
      ...geos.map(geo => ({ geo, mat: mainMat as THREE.Material, outline: false, posZ: 0 })),
      ...geos.map(geo => ({ geo, mat: outlineMat as THREE.Material, outline: true, posZ: 0 })),
    ];
  });
});
</script>
...
<template>
  <TresGroup :position="position ?? [0, 0, 0]" :scale="scale ?? 1" :rotation="[0, Math.PI, Math.PI]">
    <TresMesh
      v-for="(m, i) in meshes"
      :key="i"
      :geometry="m.geo"
